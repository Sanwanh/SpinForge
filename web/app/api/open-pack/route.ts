...
export async function POST(request: NextRequest) {
  try {
    ...
    const sparkType = `${PACKAGE_ID}::spark_token::SPARK_TOKEN`;
    const balRes = await rpc('suix_getBalance', [address, sparkType]);
    ...
    const adminCoinsRes = await rpc('suix_getCoins', [admin, sparkType, null, 1]);
