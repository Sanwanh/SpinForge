...
export function Navbar() {
  ...
  const navLabel = (id: string): string => {
    const labels: Record<string, string> = isZh
      ? { index: '首頁', register: '註冊', passport: '護照', battle: '對戰', friends: '好友', community: '攻略', cards: '收藏', workshop: '組裝', faq: '更多' }
      : { index: 'Home', register: 'Register', passport: 'Passport', battle: 'Battle', friends: 'Friends', community: 'Combos', cards: 'Collection', workshop: 'Workshop', faq: 'More' };
    return labels[id] ?? id;
  };
    ...
    <>
      ...
      <nav className="sf-mobile-bar" aria-label="Mobile primary">
        {[
          ...
          { id: 'friends',  icon: '友', href: '/friends' },
